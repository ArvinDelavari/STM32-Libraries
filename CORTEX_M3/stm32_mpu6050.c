...
MPU6050_Result_t MPU6050_Init(MPU6050_t* DataStruct, MPU6050_Device_t DeviceNumber, MPU6050_Accelerometer_t AccelerometerSensitivity, MPU6050_Gyroscope_t GyroscopeSensitivity) {
	...
	if (I2C_IsDeviceConnected(MPU6050_I2C, DataStruct->Address) != I2C_Result_Ok) {
		/* Return error */
		return MPU6050_Result_DeviceNotConnected;
	}
	
	/* Check who am I */
	I2C_Read(MPU6050_I2C, DataStruct->Address, MPU6050_WHO_AM_I, &temp);
...
MPU6050_Result_t MPU6050_SetDataRate(MPU6050_t* DataStruct, uint8_t rate) {
	...
	if (I2C_Write(MPU6050_I2C, DataStruct->Address, MPU6050_SMPLRT_DIV, rate) != I2C_Result_Ok) {
		/* Return error */
...
MPU6050_Result_t MPU6050_EnableInterrupts(MPU6050_t* DataStruct) {
	...
	I2C_Write(MPU6050_I2C, DataStruct->Address, MPU6050_INT_ENABLE, 0x21);
	
	/* Clear IRQ flag on any read operation */
	I2C_Read(MPU6050_I2C, DataStruct->Address, MPU6050_INT_PIN_CFG, &temp);
	temp |= 0x10;
	I2C_Write(MPU6050_I2C, DataStruct->Address, MPU6050_INT_PIN_CFG, temp);
...
MPU6050_Result_t MPU6050_DisableInterrupts(MPU6050_t* DataStruct) {
	/* Disable interrupts */
	if (I2C_Write(MPU6050_I2C, DataStruct->Address, MPU6050_INT_ENABLE, 0x00) != I2C_Result_Ok) {
		/* Return error */
		return MPU6050_Result_Error;
	}
...
MPU6050_Result_t MPU6050_ReadInterrupts(MPU6050_t* DataStruct, MPU6050_Interrupt_t* InterruptsStruct) {
	...
	if (I2C_Read(MPU6050_I2C, DataStruct->Address, MPU6050_INT_STATUS, &read) != I2C_Result_Ok) {
		/* Return error */
		return MPU6050_Result_Error;
	}
